...
use App\Filament\Resources\DamagedResource\Pages;
use App\Filament\Resources\DamagedResource\RelationManagers;
...
class DamagedResource extends Resource
{
    ...
    protected static ?string $modelLabel = 'Damaged Items';
                    ...
                    ->schema([
                        Split::make([
                            Section::make('Request Details')
                                ->description('The request details for the damaged item')
                                ->schema([
                                    TextEntry::make('damagedBy.full_name_en')
                                        ->label(__('Damaged By')),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->striped()
            ->recordUrl(
                fn (Model $record): string => Pages\ViewDamaged::getUrl([$record->id]),
            )
            ->columns([
            ...
            ->actions([
                ...
                Action::make('approve')
                    ...
                    ->action(function (Damaged $record) {
                        $stock = $record->stock;
                        $item = $record->item;
                        if ($record->status == 0) {
