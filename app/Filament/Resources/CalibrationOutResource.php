...
use App\Filament\Resources\CalibrationOutResource\Pages;
use App\Filament\Resources\CalibrationOutResource\RelationManagers;
use App\Models\Calibration;
use App\Models\CalibrationOut;
use App\Models\Item;
...
class CalibrationOutResource extends Resource
{
    protected static ?string $model = CalibrationOut::class;

    protected static ?string $navigationLabel = 'Calibration Out';
    protected static ?string $modelLabel = 'Calibration Out';
            ...
            ->schema([
                ...
                Repeater::make('details')
                    ...
                    ->mutateRelationshipDataBeforeCreateUsing(function (array $data, \Illuminate\Database\Eloquent\Model $record): array {
                        ...
                        $data['calibration_out_id'] = $record->id;
                        return $data;
                    })
            ]);
    }
        ...
        return $table
            ...
            ->columns([
                Tables\Columns\TextColumn::make('warehouse.name')
                    ->numeric()
                    ->sortable(),
                Tables\Columns\TextColumn::make('calibration_stock_out_date')
                    ->label('Requested At')
                    ->date()
                    ->sortable(),
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('approve')
                    ...
                    ->iconSize('lg')
                    ->action(function (CalibrationOut $record) {
                    ...
                    ->modalHeading(__('Approve Calibration Out Request'))
                    ->modalSubheading(__('Are you sure you want to approve this Calibration out request? This action cannot be undone.'))
                    ...
                    ->form(function ($record) {
                        ...
                        foreach ($details as $detail) {
                            if ($detail->items) {
                                    ...
                                    $description[$detail->item_id] = 'Check if the calibration is valid';
                            }
                        }
                        return [
                            Select::make('status')
                                ->label('Status')
                                ->options([
                                    0 => 'Not Valid Calibration',
                                    1 => 'Valid Calibration',
                                    2 => 'Partially Valid Calibration',
                                ])
                                ->required()
                                ->native(false)
                                ->reactive(),
                    ...
                    })->action(function (array $data, CalibrationOut $record) {
                    ...
                    foreach ($details as $detail) {
                        ...
                        if (in_array($detail->item_id, $items)){
                            ...
                            $calibration = Calibration::where('item_id', $detail->item_id)->first();

                            if ($calibration) {
                                if ($calibration->file && Storage::exists($calibration->file)) {
                                    Storage::delete($calibration->file);
                                }
                                ...
                                // Create a new calibration if none exists
                              $calibrationID =  Calibration::create([
                                    'item_id' => $item->id,
                                    'date' => $detail->date,
                                    'due_date' => $detail->due_date,
                                    'number' => $detail->number,
                                ...
                                $detail->update(['calibration_id' => $calibrationID->id]);
                            }
                            } else{
                    ...
                    ->modalHeading(__('Mark Items as Returned and Calibrated'))
                    ->modalSubheading(__('Are you sure you want to mark this Items as returned? And Calibrated? This action will update the stock quantities.'))
    ...
    {
        return [
            'index' => Pages\ListCalibrationOuts::route('/'),
            'create' => Pages\CreateCalibrationOut::route('/create'),
            'edit' => Pages\EditCalibrationOut::route('/{record}/edit'),
        ];
    }
}
